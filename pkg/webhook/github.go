...
func (s *githubHook) handleEvent(c *gin.Context, eventType string) {
	...
	case *github.PullRequestEvent:
		...
		repo = e.Repo.GetFullName()
		//upstreamRepo = e.PullRequest.Head.GetRepo().GetFullName()
...
func (s *githubHook) prToCheckSuite(c *gin.Context, pre *github.PullRequestEvent, proj *brigade.Project) error {
	...
	csOpts := github.CreateCheckSuiteOptions{
		...
		HeadSHA: sha,
		//HeadBranch: pre.PullRequest.GetHead().GetRef(),
		//	Name:       "Brigade testing " + pre.PullRequest.GetHead().GetSHA(),
		HeadBranch: &ref,
	}
	...
	//cs, res, err := client.Checks.CreateCheckRun(context.Background(), owner, pname, crOpts)
	if err != nil {
